...
#if !USE_MUSX
// Convert MUS to MIDI and save to file
...
static void *I_OPL_RegisterSong(void *data, int len)
{
    ...
    // For direct MIDI lump mode with standard MUS/MIDI data (not preprocessed MidX/MUSX),
    // we still need to use file-based loading to avoid the MidX format requirement
    ...
    if (IsMid(data, len)) {
        ...
        if (mus2mid(instream, outstream) == 0) {
            void *midi_buf;
            size_t midi_len;
            mem_get_buf(outstream, &midi_buf, &midi_len);
            M_WriteFile(filename, midi_buf, midi_len);
        }
